...
class customHourlyTemplateParser
{
    public static function custom_parse_template($string, $bean_arr)
    {
        foreach ($bean_arr as $bean_name => $bean_id) {
            $focus = BeanFactory::getBean($bean_name, $bean_id);
            $string = customHourlyTemplateParser::custom_parse_template_bean($string, $focus->table_name, $focus);
            ...
            foreach ($focus->field_defs as $focus_name => $focus_arr) {
                if ($focus_arr['type'] == 'relate') {
                    if (isset($focus_arr['module']) && $focus_arr['module'] != '' && $focus_arr['module'] != 'EmailAddress') {
                        ...
                        $string = customHourlyTemplateParser::custom_parse_template_bean($string, $focus_arr['name'], $relate_focus);
                    }
                }
            }
        }
    ...
    public static function custom_parse_template_bean($string, $key, &$focus)
    {
        ...
        foreach ($focus->field_defs as $field_def) {
            if (isset($field_def['name']) && $field_def['name'] != '') {
                ...
                if ($field_def['type'] == 'currency') {
                    $repl_arr[$key . "_" . $fieldName] = currency_format_number($focus->$fieldName, $params = array('currency_symbol' => false,'round'=> 2 ,'decimals' => 2, 'indian_format' => 'no'));
                    $repl_arr[$key . "_" . $fieldName] = str_pad($repl_arr[$key . "_" . $fieldName],5,"0",STR_PAD_LEFT);
        ...
        foreach ($repl_arr as $name => $value) {
            ...
            if ($name === 'aos_products_quotes_product_qty') {
                $sep = get_number_separators();
                $value = format_number($value,'2','2',$params = array('indian_format' => 'no'));
