...
function insertProductLine(tableid, groupid) {
  ...
  var vat_hidden = document.getElementById("vatproducthidden").value;
...
function insertServiceLine(tableid, groupid) {
  ...
  c.innerHTML += "<input type='hidden' name='service_sgst_vat_amt[" + servln + "]' id='service_sgst_vat_amt" + servln + "' value='0'>";
  c.innerHTML += "<input type='hidden' name='service_sgst_vat[" + servln + "]' id='service_sgst_vat" + servln + "' value='9.0'>";
  c.innerHTML += "<input type='hidden' name='service_cgst_vat_amt[" + servln + "]' id='service_cgst_vat_amt" + servln + "'value='0'>";
  c.innerHTML += "<input type='hidden'  name='service_cgst_vat[" + servln + "]' id='service_cgst_vat" + servln + "' value='9.0'>";
  if (typeof currencyFields !== 'undefined'){
    currencyFields.push("service_vat_amt" + servln);
    currencyFields.push("service_sgst_vat_amt" + servln);
    currencyFields.push("service_cgst_vat_amt" + servln);
  }
...
function calculateLine(ln, key){
  ...
  if(vat == 0 && key == 'service_') {
    document.getElementById(key + 'sgst_vat' + ln).value = '0.0';
    document.getElementById(key + 'cgst_vat' + ln).value = '0.0';
  } else if (vat == '18.0' && key == 'service_') {
    document.getElementById(key + 'sgst_vat' + ln).value = '9.0';
    document.getElementById(key + 'cgst_vat' + ln).value = '9.0';
  }
  ...
  if(key === "service_") {
    var sgst_vat = unformatNumber(document.getElementById(key + 'sgst_vat' + ln).value,',','.');
    var cgst_vat = unformatNumber(document.getElementById(key + 'cgst_vat' + ln).value,',','.');
    var sgsttotalvat=(productTotalPrice * sgst_vat) /100;
    var cgsttotalvat=(productTotalPrice * cgst_vat) /100;
    document.getElementById(key + 'sgst_vat_amt' + ln).value = format2Number(Math.round(sgsttotalvat));
    document.getElementById(key + 'cgst_vat_amt' + ln).value = format2Number(Math.round(cgsttotalvat));
  }
...
function calculateTotal(key)
{
  ...
  for (i=0; i < length; i++) {
    ...
    for (j=0; j < input.length; j++) {
      ...
      if (input[j].id.indexOf('product_vat_amt') != -1 || input[j].id.indexOf('service_vat_amt') != -1)
      {
        product_vat_amt = unformat2Number(input[j].value);
      }
      if (input[j].id.indexOf('service_sgst_vat_amt') != -1)
      {
        sgst_vat_amt = unformat2Number(input[j].value);
      }
      if (input[j].id.indexOf('service_cgst_vat_amt') != -1)
      {
        cgst_vat_amt = unformat2Number(input[j].value);
      }
    ...
    if (sgst_vat_amt !== 0 && deleted != 1) {
      sgst_tax += sgst_vat_amt;
    }
    if (cgst_vat_amt !== 0 && deleted != 1) {
      cgst_tax += cgst_vat_amt;
    }
  }
  for(var h in head){
  ...
  if(key == '') {
    set_value(key+'sgst_tax_amount',Math.round(sgst_tax));
    set_value(key+'cgst_tax_amount',Math.round(cgst_tax));
    set_value(key+'total_qty',tot_qty);
  }

  set_value(key+'total_amount',tot_amt + tax);
}
